...
{
    public partial class ManageEditDeleteNewbornfrm : Form
    {
        ...
        MySqlCommand cmd;
        MySqlDataReader dr;
        public ManageEditDeleteNewbornfrm()
        {
            InitializeComponent();
        ...
        void enableitems()
        {
            ...
            txtcc.Enabled = true;
            txtac.Enabled = true;
            amstlYES.Enabled = true;
            eincYES.Enabled = true;
            hepabYES.Enabled = true;
            bcgYES.Enabled = true;
            eyeOinthmentYes.Enabled = true;
            txtapgar.Enabled = true;
        ...
        void disableitems()
        {
            ...
            txtcc.Enabled = false;
            asss.Enabled = false;
            amstlYES.Enabled = false;
            eincYES.Enabled = false;
            hepabYES.Enabled = false;
            bcgYES.Enabled = false;
            eyeOinthmentYes.Enabled = false;
            txtapgar.Enabled = false;
            comboAttendant.Enabled = false;
        }

        private void ManageEditDeleteNewbornfrm_FormClosing(object sender, FormClosingEventArgs e)
        {
            e.Cancel = true;
            this.Hide();
        }

        private void ManageEditDeleteNewbornfrm_Load(object sender, EventArgs e)
        {
            searchPatient("");
            ...
            txtbirthwt.LostFocus += lostFocus;
        }
        ...
        private void dataGridViewNewbornPatient_RowPostPaint(object sender, DataGridViewRowPostPaintEventArgs e)
        {
        ...
        private void dataGridViewNewbornPatient_RowsAdded(object sender, DataGridViewRowsAddedEventArgs e)
        {
        }
        public void searchPatient(string searchValue)
        {
            string searchQuery = "SELECT `newbornId` as 'Newborn ID', `firstname` as 'First name', `middlename` as 'Middle name', `lastname` as 'Last name', `suffix` as 'suffix', `sex` as 'Sex' FROM `newborn_tbl` WHERE CONCAT (firstname, middlename, lastname, suffix, sex) LIKE '%" + searchValue + "%'";
            MySqlDataAdapter adapter = new MySqlDataAdapter(searchQuery, connection);
            DataTable table = new DataTable();
            adapter.Fill(table);
        ...
        public bool validateEntry(string firstname, string middlename, string lastname, string suffix)
        {
            try
            {
                ...
                cmd.CommandText = "select * from newborn_tbl where firstname='" + firstname + "' and middlename='" + middlename + "' and lastname='" + lastname + "'and suffix='" + suffix + "'";
                cmd.ExecuteNonQuery();
                DataTable table = new DataTable();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (db.insertNewborn(id, patientId, txtfirst.Text, txtmiddle.Text, txtlast.Text,txtsuffix.Text, rdMale.Checked ? "Male" : "Female", txtdateofbirth.Text, comboreligion.SelectedItem.ToString(), txtbirthwt.Text, txthc.Text, txtcc.Text, txtac.Text,amstlYES.Checked ? "1" : "0", eincYES.Checked ? "1" : "0", hepabYES.Checked ? "1" : "0", bcgYES.Checked ? "1" : "0", eyeOinthmentYes.Checked ? "1" : "0", txtapgar.Text, comboAttendant.SelectedItem.ToString(), lbldate.Text))
                {
                    db.insertInvoice(invoiceNumber,id, db.getLaborId(), "1",db.getWhatFromWhere("itemPrice", "pricetbl", "itemName","Labor"));
        ...
        void clear() 
        {
            ...
            txthc.Text = "";
            txtac.Text = "";
            amstlYES.Checked = false;
            eincYES.Checked = false;
            hepabYES.Checked = false;
            bcgYES.Checked = false;
            eyeOinthmentYes.Checked = false;
            txtapgar.Text = "";
        ...
        string updateId;
        string v1, v2, v3, v4, v5, v6, v7, v8, v9, v10, v11, v12, v13, v22, v23;
        ...
        private void eincYES_CheckedChanged(object sender, EventArgs e)
        {
        ...
        private void txtbirthwt_KeyDown(object sender, KeyEventArgs e)
        {
            if (txtbirthwt.SelectionStart == 0)
            {
                if (e.KeyValue == 32)
                {
                    e.Handled = true;
            ...
            if (e.KeyCode < Keys.D2 || e.KeyCode > Keys.D5)
            {
                e.SuppressKeyPress = true;
            }
        }
        ...
        private void txtbirthwt_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (char.IsDigit(e.KeyChar) || (char.IsControl(e.KeyChar) && e.KeyChar == Convert.ToChar(Keys.Back)) || (e.KeyChar == '.') && ((sender as TextBox).Text.IndexOf('.') > -1))
            {
                return;
            }
            e.Handled = true;
        ...
        private void dataTableMother_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            invoiceNumber = new Database().getInvoiceNumber(patientId);
            if (dr.Read()) {
        ...
        string patientId, invoiceNumber;
        ...
        private void btnEdit_Click(object sender, EventArgs e)
        {
            ...
            if (reader.Read())
            {
                ...
                txthc.Text = reader.GetValue(10).ToString();
                txtcc.Text = reader.GetValue(11).ToString();
                txtac.Text = reader.GetValue(12).ToString();
                if (reader.GetBoolean(13))
                {
                    amstlYES.Checked = true;
                }
                if (reader.GetBoolean(14))
                {
                    eincYES.Checked = true;
                }
                if (reader.GetBoolean(15))
                {
                    hepabYES.Checked = true;
                }
                ...
                {
                    eyeOinthmentYes.Checked = true;
                }
                txtapgar.Text = reader.GetValue(18).ToString();
                comboAttendant.SelectedItem = reader.GetValue(19).ToString();
            ...
            btnEdit.Enabled = false;
            btnCancel.Enabled = true;
            lbladdedit.Text = "Update Newborn Information";
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            {
                connection.Open();
                cmd = new MySqlCommand("SELECT * FROM newborn_tbl WHERE firstname = '" + txtfirst.Text.ToLower() + "' AND middlename = '" + txtmiddle.Text.ToLower() + "' AND lastname = '" + txtlast.Text.ToLower() + "'", connection);
                dr = cmd.ExecuteReader();
                if (dr.Read())
                {
                    dr.Close();
            ...
            {
                if (!(txtfirst.Text == "" || txtlast.Text == "" || txtdateofbirth.Text == "" || comboreligion.Text == "" || txtbirthwt.Text == "" ||txthc.Text == "" ||txtcc.Text == "" ||txtac.Text == "" || comboAttendant.Text == "" || txtapgar.Text == ""))
                {
                    if (db.UpdateNewborn(updateId, txtfirst.Text, txtmiddle.Text, txtlast.Text, txtsuffix.Text, gender.Text, txtdateofbirth.Text, comboreligion.SelectedItem.ToString(), patientId, txtbirthwt.Text, txthc.Text, txtcc.Text, txtac.Text, amstlYES.Checked ? "1" : "0", eincYES.Checked ? "1" : "0", hepabYES.Checked ? "1" : "0", bcgYES.Checked ? "1" : "0", eyeOinthmentYes.Checked ? "1" : "0", txtapgar.Text, comboAttendant.SelectedItem.ToString()))
                    {
        ...
        string generateUniqueID()
        {
            ...
            while (true)
            {
                ...
                cmd.CommandText = "select * from newborn_tbl where  newbornId ='" + ret + "'";
                cmd.ExecuteNonQuery();
                MySqlDataAdapter adapter = new MySqlDataAdapter(cmd);
                adapter.Fill(table);
                    MessageBox.Show(ret+"");
        ...
        {
            if (Double.Parse(txtbirthwt.Text) < 3)
                txtbirthwt.Text = "3.0";
            else if (Double.Parse(txtbirthwt.Text) >= 4.5)
                txtbirthwt.Text = "4.5";
        }
    }
